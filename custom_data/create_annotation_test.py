...
def get_keypoints(fpath):
    with torch.no_grad():
        image = np.array(Image.open(os.path.join("test", fpath))) 
        ...
        for i, point in enumerate(points):
            if point_scores[i] > 5:
                ...
                xs.append(-1)
                ys.append(-1)
        return ":".join([fpath, str(ys), str(xs)])


ann_file = open("fasion-resize-annotation-test.csv", 'w')
pair_file = open("fasion-resize-pairs-test.csv", 'w')
ann_file.write("name:keypoints_y:keypoints_x\n")
pair_file.write("from,to\n")
pair_file.write("condition.png,target.png")

fpaths = glob(sys.argv[1])
for fpath in fpaths:
    Image.open(fpath).resize((176, 256)).save(os.path.join("test", fpath))
