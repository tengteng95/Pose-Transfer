...
class PATModel_Fine(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2):
        ...
        for i in range(n_downsampling):
            ...
            model_stream1_up += [
                    nn.Sequential(
                            ResnetBlock(ngf * mult * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            ResnetBlock(ngf * mult * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            ResnetBlock(ngf * mult * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            nn.ReLU(True),
        ...
        model_stream1_up += [
                    nn.Sequential(
                            ResnetBlock(ngf * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            ResnetBlock(ngf * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            ResnetBlock(ngf * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
                            nn.ReLU(True),
    ...
    def forward(self, input): # x from stream 1 and stream 2
        ...
        for i, submodel in enumerate(self.stream1_up):
            ...
            if i==0: #3x3 
                ...
                srcBox = torso_bbox_src/2
                dstBox = torso_bbox_dst/2
                enc_fea = gen_enc_fea_all_parts_cuda(enc_fea.data, srcBox.data, dstBox.data)
            elif i==2: #7x7
